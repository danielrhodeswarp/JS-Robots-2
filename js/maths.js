...
//USE WITH Robot.scan() [and globalGame.checkRobotCollision()]
...
function colDetExplosion(explosion)	//Missiles don't hurt robots, *explosions* hurt robots ;-)
{
	for(var robotId in globalGame._robots)
	{
		...
		*/
		...
		if(smallerSquareOnBiggerSquare(globalGame._robots[robotId]._x, globalGame._robots[robotId]._y, globalGame._robots[robotId]._offset, explosion.x, explosion.y, explosion.offset))
		{
			globalGame._robots[robotId]._damage += explosion.damage;
			explosion.finished = true;
		}
